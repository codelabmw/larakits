...
it('handles connection errors', function () {
    ...
    $client->shouldReceive('get')->with(
        ...
        ['User-Agent' => 'Larakits (info@larakits.dev)']
    )->andThrow(new ConnectionException(response: new Response(
                    status: 404,
    ...
    $this->instance(Packagist::class, new Packagist(
        searchPackages: new SearchPackages(),
        client: $client,
        agent: new Agent(name: 'Larakits', email: 'info@larakits.dev')
    ));
    ...
    // Assert
    expect(Task::first()->status)->toBe('failed'); // Does not get checked if test is expecting exception
